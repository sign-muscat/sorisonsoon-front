...
const IndividualForm = () => {
    ...
    const handleSendVerificationEmail = async () => {
        try {
            const response = await axios.post('http://localhost:8080/api/v1/users/mailConfirm', {
    ...
    const handleVerifyCode = async () => {
        try {
            const response = await axios.post('http://localhost:8080/api/v1/users/verifyCode', {
